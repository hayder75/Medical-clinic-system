...
async function restoreSystemBackup() {
  try {
    ...
    // 1. Restore Users
    console.log('1️⃣  Restoring users...');
    let userCount = 0;
    for (const user of systemData.users || []) {
      try {
        ...
        console.log(`⚠️  Failed to restore user ${user.username}: ${error.message}`);
      }
    }
    console.log(`✅ Restored ${userCount} users\n`);

    // 2. Restore Services
    console.log('2️⃣  Restoring services...');
    let serviceCount = 0;
    for (const service of systemData.services || []) {
      try {
        ...
        console.log(`⚠️  Failed to restore service ${service.code}: ${error.message}`);
      }
    }
    console.log(`✅ Restored ${serviceCount} services\n`);

    // 3. Restore Investigation Types
    console.log('3️⃣  Restoring investigation types...');
    let invCount = 0;
    for (const inv of systemData.investigationTypes || []) {
      try {
        ...
        console.log(`⚠️  Failed to restore investigation ${inv.name}: ${error.message}`);
      }
    }
    console.log(`✅ Restored ${invCount} investigation types\n`);

    // 4. Restore Lab Templates
    console.log('4️⃣  Restoring lab templates...');
    let templateCount = 0;
    for (const template of systemData.labTestTemplates || []) {
      try {
        ...
        console.log(`⚠️  Failed to restore template ${template.name}: ${error.message}`);
      }
    }
    console.log(`✅ Restored ${templateCount} lab templates\n`);

    // 5. Restore Departments
    console.log('5️⃣  Restoring departments...');
    let deptCount = 0;
    for (const dept of systemData.departments || []) {
      try {
        ...
        console.log(`⚠️  Failed to restore department ${dept.name}: ${error.message}`);
      }
    }
    console.log(`✅ Restored ${deptCount} departments\n`);

    // 6. Restore Insurances
    console.log('6️⃣  Restoring insurances...');
    ...
    for (const insurance of systemData.insurances || []) {
      try {
        ...
        console.log(`⚠️  Failed to restore insurance ${insurance.name}: ${error.message}`);
      }
    }
    console.log(`✅ Restored ${insuranceCount} insurances\n`);

    // 7. Restore Medication Catalog
    console.log('7️⃣  Restoring medication catalog...');
    let medCount = 0;
    for (const med of systemData.medicationCatalog || []) {
      try {
        ...
        console.log(`⚠️  Failed to restore medication ${med.name}: ${error.message}`);
      }
    }
    console.log(`✅ Restored ${medCount} medications\n`);

    // 8. Restore Inventory
    console.log('8️⃣  Restoring inventory...');
    let invItemCount = 0;
    for (const item of systemData.inventory || []) {
      try {
        ...
        console.log(`⚠️  Failed to restore inventory item ${item.name}: ${error.message}`);
      }
    }
    console.log(`✅ Restored ${invItemCount} inventory items\n`);

    // 9. Restore System Settings
    if (systemData.systemSettings && systemData.systemSettings.length > 0) {
      console.log('9️⃣  Restoring system settings...');
      let settingsCount = 0;
      for (const setting of systemData.systemSettings) {
        try {
          ...
          console.log(`⚠️  Failed to restore setting ${setting.key}: ${error.message}`);
        }
      }
      console.log(`✅ Restored ${settingsCount} system settings\n`);
    }

    // 10. Restore Teeth Data
    if (systemData.teeth && systemData.teeth.length > 0) {
      console.log('🔟 Restoring teeth data...');
      let teethCount = 0;
      for (const tooth of systemData.teeth) {
        try {
          ...
          console.log(`⚠️  Failed to restore tooth ${tooth.number}: ${error.message}`);
        }
      }
      console.log(`✅ Restored ${teethCount} teeth records\n`);
    }

    console.log('='.repeat(60));
    console.log('🎉 System Backup Restored Successfully!');
    console.log('='.repeat(60));
    console.log('\n📊 Summary:');
