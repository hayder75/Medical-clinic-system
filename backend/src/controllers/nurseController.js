...
exports.recordVitals = async (req, res) => {
  try {
    ...
    // Update visit status if it's WAITING_FOR_TRIAGE or TRIAGED - move to IN_DOCTOR_QUEUE after triage is filled
    // This moves the patient from triage queue to main doctor queue where orders can be saved
    if (visit.status === 'WAITING_FOR_TRIAGE' || visit.status === 'TRIAGED') {
      // Check if this is a doctor recording vitals (triage)
      const isDoctor = req.user && req.user.role === 'DOCTOR';
      
      if (isDoctor) {
        // Doctor is filling triage - move to main doctor queue
        const doctorId = req.user.id;
        
        // Find or create assignment for this doctor and patient
        ...
        if (!assignment) {
          ...
          console.log('🔍 recordVitals: Created new assignment for doctor', doctorId, 'and patient', data.patientId);
        }

        const updateData = {
          status: 'IN_DOCTOR_QUEUE' // Move to main queue
        };

        // Link assignment to visit if not already linked
        if (!visit.assignmentId) {
          updateData.assignmentId = assignment.id;
          console.log('🔍 recordVitals: Linked assignment', assignment.id, 'to visit', data.visitId);
        }
        ...
        console.log('🔍 recordVitals: Doctor filled triage - Updated visit status from', visit.status, 'to IN_DOCTOR_QUEUE for visitId:', data.visitId);
      } else {
        // Nurse is recording vitals - keep in triage or move to TRIAGED
        await prisma.visit.update({
          where: { id: data.visitId },
          data: { status: 'TRIAGED' }
        });
        console.log('🔍 recordVitals: Nurse recorded vitals - Updated visit status from', visit.status, 'to TRIAGED for visitId:', data.visitId);
      }
    } else {
    ...
    for (const task of nurseServiceTasks) {
      ...
      if (billing) {
        ...
        if (isFullyPaid) {
          ...
          const servicePrice = task.isWaived ? 0 : (task.service.price || 0);
          groupedTasks[patientId].services.push({
            ...
            originalPrice: task.service.price || 0, // Keep original price for reference
...
exports.getWalkInNurseOrders = async (req, res) => {
  try {
    ...
    // Get paid walk-in orders (all nurses can see all unassigned orders)
    // Orders without a nurse assigned (nurseId: null) are available to all nurses
    // Orders with a nurse assigned are only visible to that nurse
    const orders = await prisma.nurseWalkInOrder.findMany({
      where: {
        status: 'PAID',
        // Show orders that are unassigned (null) OR assigned to current nurse
        OR: [
          { nurseId: null },
...
exports.completeWalkInNurseOrder = async (req, res) => {
  try {
    ...
    // Update order status and assign nurse if not already assigned
    const updatedOrder = await prisma.nurseWalkInOrder.update({
      where: { id: orderId },
      data: {
        status: 'COMPLETED',
        completedAt: new Date(),
        nurseId: order.nurseId || nurseId, // Assign current nurse if not already assigned
