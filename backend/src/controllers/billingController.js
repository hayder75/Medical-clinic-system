    ...
    if (isFullyPaid) {
      ...
      if (billing.visit) {
        await prisma.$transaction(async (tx) => {
          ...
          // Check if this is dental services billing
          const isDentalBilling = billing.services.some(service => 
            service.service.category === 'DENTAL'
          ) || billing.visit.status === 'DENTAL_SERVICES_ORDERED';
          
          if (isDentalBilling || billing.visit.status === 'DENTAL_SERVICES_ORDERED') {
            // Update dental batch orders
            ...
            // Return patient to doctor queue after payment
            await tx.visit.update({
              where: { id: billing.visit.id },
              data: { status: 'IN_DOCTOR_QUEUE' }
            });
          } else if (isDiagnosticsBilling) {
            console.log('💰 [Payment] Updating doctor lab orders for visit:', billing.visit.id);
            
            // Check how many orders exist before update
            const ordersBeforeUpdate = await tx.labTestOrder.count({
              where: {
                visitId: billing.visit.id
              }
            });
            ...
            console.log(`   ✅ Updated ${batchOrdersUpdated.count} batch orders to QUEUED`);

            // Update batch order services
            const batchServicesUpdated = await tx.batchOrderService.updateMany({
              where: {
                batchOrder: {
                  visitId: billing.visit.id
                },
            ...
            console.log(`   ✅ Updated ${batchServicesUpdated.count} batch order services to QUEUED`);

            // Update new system LabTestOrders (for doctor orders)
            // IMPORTANT: Only update orders linked to THIS specific billing to avoid updating orders from other billings
            const labTestOrdersUpdated = await tx.labTestOrder.updateMany({
              where: {
                billingId: billing.id, // Only update orders linked to this billing
                visitId: billing.visit.id,
                status: 'UNPAID'
              },
              data: { status: 'PAID' }
            });
            console.log(`   ✅ Updated ${labTestOrdersUpdated.count} new lab test orders to PAID (billingId: ${billing.id})`);

            // Verify update worked
            if (labTestOrdersUpdated.count === 0 && ordersBeforeUpdate > 0) {
              console.warn('   ⚠️  WARNING: Orders exist but update count is 0. Checking order statuses...');
              const existingOrders = await tx.labTestOrder.findMany({
                where: {
                  visitId: billing.visit.id
                },
                select: { id: true, status: true }
            ...
            const oldLabOrdersUpdated = await tx.labOrder.updateMany({
              where: {
                visitId: billing.visit.id,
                status: 'UNPAID'
              },
            ...
            const radOrdersUpdated = await tx.radiologyOrder.updateMany({
              where: {
                visitId: billing.visit.id,
                status: 'UNPAID'
              },
