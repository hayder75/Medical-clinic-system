...
const TriageQueue = () => {
  ...
  const [expandedSections, setExpandedSections] = useState({
    vitals: false,        // Start collapsed - user clicks to expand
    complaint: false,
    examination: false,
    nurseService: false,
    dentalService: false,
  ...
  useEffect(() => {
    if (selectedPatient?.bloodType && selectedPatient.bloodType !== 'UNKNOWN') {
      // Pre-fill blood type if patient already has one recorded
      setVitalsData(prev => ({
        ...prev,
        bloodType: selectedPatient.bloodType
      }));
    } else {
      // Clear blood type if patient doesn't have one or it's UNKNOWN
      setVitalsData(prev => ({
        ...prev,
        bloodType: ''
      }));
    ...
    try {
      ...
      if (sectionCompletion.nurseService && sectionCompletion.assignment) {
        const payload = {
          ...
          waivedServiceIds: waivedServiceIds, // Add waived services
        ...
        // Handle services assignment only (nurse + dental)
        const payload = {
          visitId: selectedPatient.id,
          patientId: selectedPatient.patient.id,
          serviceIds: allServiceIds,
              ...
              <form onSubmit={handleVitalsSubmit} className="space-y-4">
                ...
                <div className="border border-gray-200 rounded-lg">
                  <SectionHeader
                    ...
                    title="Nurse Services (Optional)"
                    icon={Stethoscope}
                    isCompleted={selectedNurseServices.length > 0}
                ...
                <div className="border border-gray-200 rounded-lg">
                  <SectionHeader
                    section="dentalService"
                    title="Dental Services (Optional)"
                    icon={Stethoscope}
                    isCompleted={selectedDentalServices.length > 0}
                    isExpanded={expandedSections.dentalService}
                    onToggle={toggleSection}
                  />
                  
                  {expandedSections.dentalService && (
                    <div className="p-6 border-t border-gray-200">
                      <div className="space-y-4">
                        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                          <p className="text-sm text-blue-800">
                            <strong>Note:</strong> Select dental services. For services priced per unit (tooth, jaw, etc.), enter the quantity.
                          </p>
                        </div>
                          ...
                          <div className="space-y-2 max-h-60 overflow-y-auto border border-gray-200 rounded-lg p-3">
                            {dentalServices
                              ...
                              .map(service => {
                                ...
                                const selectedService = selectedDentalServices.find(s => s.serviceId === service.id);
                                const isWaived = waivedDentalServices.has(service.id);
                                const isFree = service.price === 0 || service.price === null;
                                const displayPrice = isWaived ? 0 : (service.price || 0);
                                const totalPrice = displayPrice * (selectedService?.quantity || 1);
                                const unitLabel = service.unit === 'TOOTH' ? 'per tooth' : 
                                                 service.unit === 'JAW' ? 'per jaw' : 
                                                 service.unit === 'ABUTMENT' ? 'per abutment' : 
                                                 service.unit === 'SESSION' ? 'per session' : 
                                                 service.unit === 'SEGMENT' ? 'per segment' : 
                                                 service.unit === 'UNIT' ? '' : service.unit?.toLowerCase() || '';
                                
                                return (
                                  <div key={service.id} className={`border rounded-lg p-3 hover:bg-gray-50 ${isSelected ? 'border-blue-300 bg-blue-50' : 'border-gray-200'}`}>
                                    <div className="flex items-start space-x-3">
                                      ...
                                      <div className="flex-1">
                                        ...
                                        {/* Quantity Input - Show when selected, hide when unselected */}
                                        {isSelected && (
                                          <div className="mt-2 p-2 bg-blue-50 rounded border border-blue-200">
                                            ...
                                            <div className={`mt-1 text-xs font-medium ${isWaived ? 'text-green-700' : 'text-green-700'}`}>
                                              Total: {unitLabel ? `${selectedService?.quantity || 1} × ` : ''}ETB {displayPrice.toLocaleString()} = ETB {totalPrice.toLocaleString()}
                                            </div>
                                          </div>
                                        )}
                                      </div>
                          ...
                          {selectedDentalServices.length > 0 && (
                            <div className="mt-3 p-3 bg-green-50 rounded-lg">
                              ...
                              <div className="space-y-1">
                                {selectedDentalServices.map((selected, idx) => {
                                  const service = dentalServices.find(s => s.id === selected.serviceId);
                                  if (!service) return null;
                                  const total = (service.price || 0) * (selected.quantity || 1);
                                  return (
                                    <div key={idx} className="text-xs text-gray-700">
                                      • {service.name} × {selected.quantity} = ETB {total.toLocaleString()}
                                    </div>
                                  );
                                })}
                              </div>
                        ...
                        {/* Nurse Assignment for Dental Services */}
                        {selectedDentalServices.length > 0 && (
                          <div>
                            <label className="label">Assign to Nurse</label>
                            <select
                        ...
                        {/* Service Notes for Dental Services */}
                        {selectedDentalServices.length > 0 && selectedNurse && (
                          <div>
                            <label className="label">Service Notes (Optional)</label>
                            <textarea
                              className="input"
                ...
                {selectedPatient && (
                  <div className="border border-gray-200 rounded-lg">
                    <SectionHeader
                      section="materialNeeds"
                      title="Material Needs (Optional)"
                      icon={Package}
                      isCompleted={false}
                      isExpanded={expandedSections.materialNeeds}
                      onToggle={toggleSection}
                    />
                    
                    {expandedSections.materialNeeds && (
                      <div className="p-6 border-t border-gray-200">
                ...
                <div className="bg-gray-50 p-4 rounded-lg">
                  ...
                  <div className="flex space-x-4 text-sm">
                    ...
                    <span className={`flex items-center ${sectionCompletion.dentalService ? 'text-green-600' : 'text-gray-500'}`}>
                      <CheckCircle className="h-4 w-4 mr-1" />
                      Dental Service
                    </span>
