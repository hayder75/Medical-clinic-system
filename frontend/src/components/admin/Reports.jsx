      ...
      <html>
        ...
        <body>
          <div class="header">
            ...
            <div class="report-title">Financial Report - ${revenueType.toUpperCase()}</div>
            <div>Period: ${selectedPeriod === 'daily' ? selectedDate : `${getMonthName(selectedMonth)} ${selectedYear}`}</div>
            <div>Generated: ${new Date().toLocaleString()}</div>
          </div>
          <div class="summary">
            <h3>Summary</h3>
            <p><strong>Total Revenue:</strong> ${formatCurrency(revenueData.revenue)}</p>
            <p><strong>Total Transactions:</strong> ${revenueData.transactions}</p>
            ${pendingData ? `<p><strong>Pending Revenue:</strong> ${formatCurrency(pendingData.revenue)}</p>` : ''}
          </div>
              ...
              <tbody>
                ${daysWithData.map(day => `
                  <tr>
                    ...
                    <td>${formatCurrency(day.combined?.revenue || 0)}</td>
                  </tr>
                `).join('')}
              </tbody>
            </table>
