...
import { UserPlus, User, Phone, TestTube, X, Check, AlertCircle } from 'lucide-react';
...
const WalkInOrders = () => {
  ...
  const fetchAvailableTests = async () => {
    try {
      ...
      setAvailableTests(response.data.services || []);
    } catch (error) {
            ...
            {availableTests.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 max-h-96 overflow-y-auto">
                {availableTests.map(test => (
                  <label key={test.id} className={`flex items-center p-3 border rounded-lg cursor-pointer transition-colors ${selectedTests.includes(test.id) ? 'border-blue-500 bg-blue-50' : 'border-gray-300 hover:border-gray-400'}`}>
                    <input type="checkbox" checked={selectedTests.includes(test.id)} onChange={() => toggleTestSelection(test.id)} className="mr-3 h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded" />
